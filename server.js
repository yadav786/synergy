...
app.route('/users').   
 ...
 post((req, res)=>{      
 	if(req.query.username && req.query.age && req.query.address){ 
 		console.log('sdfsdfdfsa'); 
 		allUsers.push({username:req.query.username, age:req.query.age, id:Math.random(), address:req.query.address});
 		session = allUsers;     
 	}       
	res.status(200).send({errorCode:false, data:session, errorMessage:'Data Inserted Successfully'});  
 }).        
 delete((req, res) => {   
 	const deletedData = session;
 	session = deletedData.filter(value => value.id !== req.query.id);  
 	allUsers = session;    
	res.status(200).send({errorCode:false, data:session, errorMessage:'Data Deleted Successfully'});   
 });   
