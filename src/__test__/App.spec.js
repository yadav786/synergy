...
describe('Provider ...', () => {
  ...
  let mountWrapper; let shallowWrapper; let renderWrapper

  beforeEach(() => {
  	 // shallowWrapper = shallow(<Provider store={store}><App /></Provider>)
  	 mountWrapper = mount(<Provider store={store}><App /></Provider>)
  ...
  it('Provider rendering  `Provider` ...', () => {
    ...
    // const callback = spy()
    console.log('\x1b[33m mountWrapper LogIn  Debug ===', mountWrapper.find(LogIn).debug())
    console.log('\x1b[33m mountWrapper LogIn  ===', JSON.stringify(mountWrapper.find(LogIn)) + '\x1b[0m')
    // console.log('\x1b[32m mountWrapper LogIn Form ===', mountWrapper.find(LogIn).find('Form').debug() + '\x1b[0m')
    console.log('\x1b[32m mountWrapper LogIn Form ===', mountWrapper.find(LogIn).find('Button').simulate('click') + '\x1b[0m')
